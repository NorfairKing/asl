...
sshRules = do
    sshRule ~>
        need [customSshKeyFile, customSshPublicKeyFile, customSshConfigFile, customKnownHostsFile]
    ...
    customKnownHostsFile %> \_ -> writeFile' customKnownHostsFile ""
    customSshConfigFile %> \_ -> do
        need [customKnownHostsFile, customSshPublicKeyFile]
        writeFile' customSshConfigFile $
            unlines
                [ "Host *"
