...
traceSliceAnalysisRules = do
    traceSliceAnalysisRule ~> need [ruleForMaximumThroughputs]
    subRules rulesForTraceSliceAnalysis ruleForMaximumThroughputs allMaximumThroughputExperiments
...
useTraceSlicePlotsInReport ecf i = traceSlicePlotsFor ecf >>= (`usePlotsInReport` i)

dependOnTraceSlicePlotsForReport :: MaximumThroughputCfg -> Int -> Action ()
dependOnTraceSlicePlotsForReport ecf i = traceSlicePlotsFor ecf >>= (`dependOnPlotsForReport` i)

traceSlicePlotsFor
    :: MonadIO m
    => MaximumThroughputCfg -> m [FilePath]
...
rulesForTraceSliceAnalysis ecf =
    onlyIfResultsExist ecf $
    ...
    -- TODO need averages
        ...
        adFile %> \outFile -> do
            need [dFile]
            putLoud $ unwords ["Gathering absolute trace slice data from", dFile, "into", outFile]
            transformCsvFileAction dFile outFile absLineTransformer
        rdFile %> \outFile -> do
            need [dFile]
            putLoud $ unwords ["Gathering relative trace slice data from", dFile, "into", outFile]
...
avgDurationsFile ecf =
    experimentAnalysisTmpDir ecf </> experimentTarget ecf ++ "-avg-durations" <.> csvExt

absDurationsFile :: MaximumThroughputCfg -> FilePath
absDurationsFile ecf =
    experimentAnalysisTmpDir ecf </> experimentTarget ecf ++ "-absolute-durations" <.> csvExt

relDurationsFile :: MaximumThroughputCfg -> FilePath
relDurationsFile ecf =
    experimentAnalysisTmpDir ecf </> experimentTarget ecf ++ "-relative-durations" <.> csvExt
