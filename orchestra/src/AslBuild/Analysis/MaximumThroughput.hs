...
throughputAnalysisRules = do
    throughputAnalysisRule ~> need [ruleForMaximumThroughputs]
...
ruleForThroughputAnalysis
    :: ExperimentConfig a
    => a -> String
ruleForThroughputAnalysis mtc = experimentTarget mtc ++ "-throughput-analysis"
...
useThroughputAnalysisPlotsInReport stc = usePlotsInReport $ throughputAnalysisPlotsFor stc
...
rulesForThroughputAnalysis mtc =
    onlyIfResultsExist mtc $ do
        ...
        -- TODO combine repititions
         -> do
            ...
            ls <-
                forP (concat summaryPaths) $ \summaryPath -> do
...
simplifiedCsvLine :: ExperimentSetup -> MemaslapClientResults -> MTSimpleLine
...
data MTSimpleLine = MTSimpleLine
    { middleThreads :: Int
...
instance ToNamedRecord MTSimpleLine where
    toNamedRecord MTSimpleLine {..} =
        namedRecord ["threads" .= middleThreads, "conc" .= clientConcurrencies] <>
        mapKeys (<> "Resp") (toNamedRecord responseTime) <>
        mapKeys (<> "Tps") (toNamedRecord throughput)

instance DefaultOrdered MTSimpleLine where
    headerOrder _ =
        header ["threads", "conc"] <> V.map (<> "Resp") (headerOrder (undefined :: Avg)) <>
