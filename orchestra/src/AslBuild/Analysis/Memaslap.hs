...
memaslapLogRulesForRepSet ecf slocs = do
    ...
    combinedRepsetResultsFile %> \_ -> do
        ...
        putLoud $
            "Combining combined client result files " ++
            show combinedResultsFiles ++ " into " ++ combinedRepsetResultsFile
...
readCombinedClientsResults
    :: MonadIO m
    => FilePath -> m CombinedClientResults
readCombinedClientsResults = readJSON

memaslapLogsRulesForRep
    :: ExperimentConfig a
    => a -> FilePath -> Rules [FilePath]
memaslapLogsRulesForRep ecf sloc = do
    ...
    combinedResultsFile %> \_ -> do
        need resultsFiles
        putLoud $
            "Combining client result files " ++ show resultsFiles ++ " into " ++ combinedResultsFile
        rs <-
            forP resultsFiles $ \resultsFile -> do
...
combinedClientRepsetResultsFile
    :: ExperimentConfig a
    => a -> [FilePath] -> FilePath
combinedClientRepsetResultsFile ecf fs =
    (`replaceSndDir` localClientResultsDir ecf) .
    changeFilename (const $ "combined-repset-results-" ++ takeBaseName (head fs)) . head $
    fs

combineClientResultsFile
    :: ExperimentConfig a
    => a -> FilePath -> FilePath
combineClientResultsFile ecf =
    (`replaceSndDir` localClientResultsDir ecf) . changeFilename (++ "-combined-results")

readCombinedClientResults
    :: MonadIO m
    => FilePath -> m MemaslapClientResults
readCombinedClientResults = readJSON

combineClientResults :: [MemaslapClientResults] -> MemaslapClientResults
combineClientResults cs =
...
combineRespAvgs as = Avg {avg = avgAvg $ map avg as, stdDev = combineStdDevs $ map stdDev as}

combineTpsAvgResults :: [AvgResults] -> AvgResults
combineTpsAvgResults rs =
    AvgResults
...
combineTpsAvgs as = Avg {avg = sum $ map avg as, stdDev = combineStdDevs $ map stdDev as}
...
pureClientResults sts =
    let mkAvgS :: (Statistics -> Int) -> [StatisticsLog] -> Avg
        mkAvgS func sl = Avg {avg = S.mean vec, stdDev = S.stdDev vec}
          where
            vec = V.fromList $ map (fromIntegral . func . periodStats) sl
