...
data MetaAvgResults = MetaAvgResults
...
instance Num MetaAvg where
    (MetaAvg aa1 sa1 ss1) + (MetaAvg aa2 sa2 ss2) =
        MetaAvg
        { avgAvgs = aa1 + aa2
...
instance ToNamedRecord MetaAvg where
    toNamedRecord MetaAvg {..} =
        namedRecord ["avgavg" .= avgAvgs, "avgstd" .= stdDevAvgs, "combstd" .= combStdDev]

instance DefaultOrdered MetaAvg where
    headerOrder _ = header ["avgavg", "avgstd", "combstd"]

data AvgResults = AvgResults
...
instance ToNamedRecord Avg where
    toNamedRecord Avg {..} = namedRecord ["avg" .= avg, "std" .= stdDev]
