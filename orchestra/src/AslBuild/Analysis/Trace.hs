...
durationsRulesForRepset
    :: ExperimentConfig a
    => a -> [FilePath] -> Rules [FilePath]
...
combinedDursFileRules
    :: ExperimentConfig a
    => a -> [ExperimentResultSummary] -> Rules [FilePath]
combinedDursFileRules ecf erss =
    case mapM merMiddleResultsFile erss of
        ...
        Just mes -> do
            ...
            f3 <-
                asCombinedAvgDurationsFile combinedAvgWriteDurationFile avgWriteDurationFile ecf mes
            return [f1, f2, f3]
...
combineAvgDurs das =
  ...
  where
    mkMetaAvg func = metaAvg $ map func das

durationsRulesForExperimentResults
    :: ExperimentConfig a
    => a -> ExperimentResultSummary -> Rules [FilePath]
durationsRulesForExperimentResults ecf er =
    case merMiddleResultsFile er of
        Nothing -> pure []
        Just erMiddleResultsFile -> durationsRulesForMiddleResults ecf erMiddleResultsFile

durationsRulesForMiddleResults
    :: ExperimentConfig a
    => a -> FilePath -> Rules [FilePath]
durationsRulesForMiddleResults ecf erMiddleResultsFile = do
    ...
    totalDur %> \outFile -> do
        ...
        transformCsvFileAction erMiddleResultsFile outFile $
            P.map $ \mrl -> TotalDuration $ requestRespondedTime mrl - requestReceivedTime mrl
...
timeTransformer
    :: Monad m
    => Pipe MiddleResultLine MiddleDurationsLine m v
timeTransformer = do
    mrl <- P.await
...
readAvgDurationsFile
    :: MonadIO m
    => FilePath -> m (Durations Avg)
...
calcAvgDur path = do
    let withFold fold =
            liftIO $
            withFile path ReadMode $ \inHandle -> do
                let prod =
                        P.decodeByName (PB.fromHandle inHandle) >-> errorIgnorer >-> P.map durations >->
...
rawDurationsFile
    :: ExperimentConfig a
    => a -> FilePath -> FilePath
rawDurationsFile ecf =
    changeFilename (++ "-durations") . (`replaceSndDir` experimentAnalysisTmpDir ecf)
...
avgDurationFile
    ...
    => a -> FilePath -> FilePath
avgDurationFile ecf =
    changeFilename (++ "-average") . (<.> jsonExt) . dropExtensions . rawDurationsFile ecf
