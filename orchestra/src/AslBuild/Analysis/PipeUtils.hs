...
transformCsvFile
    :: forall m a b.
       (MonadIO m, FromNamedRecord a, ToNamedRecord b, DefaultOrdered b)
    => FilePath -> FilePath -> Pipe a b IO () -> m ()
transformCsvFile infile outfile pipe =
    liftIO $
    withFile infile ReadMode $ \inHandle ->
        withFile outfile WriteMode $ \outHandle ->
            P.runEffect $
            P.decodeByName (PB.fromHandle inHandle) >-> errorIgnorer >-> pipe >->
            P.encodeByName (CSV.headerOrder (undefined :: b)) >->
            PB.toHandle outHandle

transformCsvFileAction
    :: forall a b.
       (FromNamedRecord a, ToNamedRecord b, DefaultOrdered b)
    => FilePath -> FilePath -> Pipe a b IO () -> Action ()
