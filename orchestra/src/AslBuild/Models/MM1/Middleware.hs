...
mm1MiddlewareModelFileFor
    :: ExperimentConfig a
    => a -> FilePath -> FilePath
mm1MiddlewareModelFileFor ecf =
    changeFilename (++ "-mm1-middleware") . (`replaceDirectory` experimentAnalysisTmpDir ecf)

calcMiddlewareMM1Model
    :: MonadIO m
    => FilePath -> m MM1Model
...
calcMiddlewareArrivalAvg
    :: MonadIO m
    => ExperimentSetup -> FilePath -> m Avg
calcMiddlewareArrivalAvg setup traceFile
 ...
 = do
    ...
    (rsr, wsr) <-
        case (,) <$> mwReadSampleRate mwf <*> mwWriteSampleRate mwf of
...
calcMiddlewareServiceRateAvg
    :: MonadIO m
    => FilePath -> m Avg
calcMiddlewareServiceRateAvg =
    averageInMiddlewareOf $ \mrl ->
        1 / (fromIntegral (requestRepliedTime mrl - requestDequeuedTime mrl) / (1000 * 1000 * 1000))

averageInMiddlewareOf
    :: MonadIO m
    => (MiddleResultLine -> Double) -> FilePath -> m Avg
averageInMiddlewareOf projection = avgInMiddlewareWithFold $ P.map projection

bucketizedInMiddlewareAverageOf
    :: MonadIO m
    => Double -> (MiddleResultLine -> Integer) -> FilePath -> m Avg
...
avgInMiddlewareWithFold
    :: MonadIO m
    => Pipe MiddleResultLine Double IO () -> FilePath -> m Avg
avgInMiddlewareWithFold pipe traceFile = do
    let withFold fold =
            liftIO $
            withFile traceFile ReadMode $ \inHandle -> do
                let prod = P.decodeByName (PB.fromHandle inHandle) >-> errorIgnorer >-> pipe
                fold prod
    a <-
        withFold $ \prod -> do
...
bucketizer
    :: Monad m
    => (MiddleResultLine -> Integer) -> Pipe MiddleResultLine [MiddleResultLine] m v
