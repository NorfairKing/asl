...
mm1Rules :: Rules ()
mm1Rules = do
    mm1Rule ~> need [ruleForStabilityTraces, ruleForMaximumThroughputs]
    subRules mm1RulesFor ruleForStabilityTraces allStabilityTraceExperiments
    subRules mm1RulesFor ruleForMaximumThroughputs allMaximumThroughputExperiments
...
mm1ModelTexFilePrefix
    :: ExperimentConfig a
    => a -> FilePath
mm1ModelTexFilePrefix ecf = reportsTmpDir </> experimentTarget ecf ++ "-mm1" <.> texExt

mm1ModelTexFileWithPostfix
    :: ExperimentConfig a
    => a -> String -> FilePath
mm1ModelTexFileWithPostfix ecf postfix =
    changeFilename (++ "-" ++ postfix) $ mm1ModelTexFilePrefix ecf
...
useMM1ModelInReport
    :: ExperimentConfig a
    => a -> Int -> Rules ()
useMM1ModelInReport ecf i =
    forM_ (mm1ModelTexFiles ecf) $ \eff -> mm1ModelFileForReport eff i `byCopying` eff

dependOnMM1ModelForReport
    :: ExperimentConfig a
    => a -> Int -> Action ()
...
mm1EstimationRuleFor
    :: ExperimentConfig a
    => a -> String
mm1EstimationRuleFor ecf = experimentTarget ecf ++ "-mm1-estimation"

mm1ModelEstimateFileFor
    :: ExperimentConfig a
    => a -> [FilePath] -> FilePath
mm1ModelEstimateFileFor ecf =
    changeFilename (const "mm1-estimate") . (`replaceSndDir` experimentAnalysisTmpDir ecf) . head

mm1EstimationRulesFor
    :: ExperimentConfig a
    => a -> Rules String
    ...
    forP_ slocss $ \slocs -> do
        ...
        let t1 =
                tabularWithHeader
                    ...
                    , dline
                          "Jobs in queue/jobs in system"
                          (mm1WaitingJobs mm1 / mm1MeanNrJobs mm1)
                          ""
                    ]
        ...
        let t2 =
                tabularWithHeader
                    ...
                    , dline
                          "Std Dev time in middleware"
                          (timeFromMiddle actualStdDevResp)
                          "$\\mu s$"
                    ...
                    , [ "Jobs in the queue"
                      , unwords
                            ...
                            , show totalNrWorkers
                            , ") ="
                            , show $ totalNrClients - totalNrWorkers
                            , "$"
                            ]
                      , "Jobs"
                      ]
                    , dline
                          "Jobs in queue/jobs in system"
                          (fromIntegral (totalNrClients - totalNrWorkers) /
                           fromIntegral totalNrClients)
                          ""
                    ]
