...
genLogfileListingFor
    :: ExperimentConfig a
    => a -> String
genLogfileListingFor ecf =
    unlines $
    map
        (inHline .
         uncurry
             (\a b ->
                  a ++
                  " & {\\footnotesize \\url{https://gitlab.inf.ethz.ch/" ++
                  myNetzh ++ "/asl-fall16-project/blob/master/" ++ b ++ "}}"))
...
makeCompleteListingWithLines cs =
    unlines $
    ["\\begin{tabular}{c|p{8cm}}", "\\textbf{Short name }& \\textbf{Location} \\\\"] ++
    cs ++ ["\\end{tabular}"]
