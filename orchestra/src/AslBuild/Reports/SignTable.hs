...
signTableFiles ecf = do
    ...
    extra <-
        ["results", "legend"] ++
        [intercalate "-" [a, b] | a <- ["add", "mul"], b <- ["signtable", "variations"]]
...
useSignTableInReport
    :: ExperimentConfig a
    => a -> Int -> Rules ()
useSignTableInReport ecf i =
    forM_ (signTableFiles ecf) $ \eff -> signTableFileForReport eff i `byCopying` eff

dependOnSignTableForReport
    :: ExperimentConfig a
    => a -> Int -> Action ()
...
genTpsSignTable =
    genSignTableWith tpsResults avgTpsResults id "tps" "Throughput (transactions / second)"

genRespSignTable :: FactorialCfg -> Action ()
genRespSignTable =
    genSignTableWith respResults avgRespResults (/ 100) "resp" "Response Time ($100 \\mu s$)"
...
genResultsTable funcRes funcAvgRes convFunc measSuf ecf = do
    ...
    let resultsRows =
            flip map realRes $ \(indivs, res) -> [show $ map roundD indivs, printf "%.2f" res]
    let resultsTable =
            tabularWithHeaderAndAllignment
                [AllCenter, AllCenter, AllCenter, AllCenter, AllRight]
                resultsHeader
...
genSingleSignTable funcRes funcAvgRes convFunc measSuf ecf _ _ suffix = do
    ...
    let effects :: [Double]
        effects =
            flip map signColumns $ \signColumn -> sum $ mult (map fromIntegral signColumn) resvec
        effDivs :: [Double]
    ...
    let rows =
            flip map (zip (zip signRowSs tups) errss) $ \((row, (_, res)), errs) ->
    ...
    let signTable = tabularWithHeader header table
    let tableFile = signTableFileWithPostfix ecf $ intercalate "-" [measSuf, suffix, "signtable"]
    writeFile' tableFile signTable
    let predTable =
            tabularWithHeaderAndAllignment
                [AllCenter, AllRight, AllRight]
                ["Factor", "Variation", "Percentage"]
                (transpose
                     [ drop 1 headerPrefix ++ ["Error"]
                     , map (show . roundD) (variations ++ [sse])
                     , map showPerc (relvars ++ [sse / sst])
                     ])
    let predictionFile =
            signTableFileWithPostfix ecf $ intercalate "-" [measSuf, suffix, "variations"]
