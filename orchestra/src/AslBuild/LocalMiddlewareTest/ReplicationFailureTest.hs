...
data ReplicationFailureTestSetup = ReplicationFailureTestSetup
    { cPort :: Int
    , mPort :: Int
    , sPortBase :: Int
    , nrServers :: Int
...
setups = do
    ...
    return
        ...
        {cPort = 11272, mPort = 11273, sPortBase = 11274, nrServers = nSers, err = e}
    ...
    localMiddlewareReplicationFailureTestRule ~> do
        ...
        forM_ setups $ \ReplicationFailureTestSetup {..} -> do
            ...
            let mwFlags =
                    ...
                    , mwVerbosity = LogAll
                    , mwTraceFile =
                          tmpDir </> localMiddlewareReplicationFailureTestRule ++
                          "-trace" <.> csvExt
                            ...
                            case mres of
                                ...
                                Just res ->
                                    ...
                                    unlines
                                        [ "On input: " ++ show req
                                        , "the middleware sent " ++
                                          show res ++ " to the server instead."
                                        ]
                        let errdat =
                                case err of
                        ...
                        case mres2 of
                            ...
                            Just res2 ->
                                ...
                                unlines
                                    ...
                                    , "the middleware sent " ++
                                      show res2 ++ " to the client instead."
                                    ]
            let tests = do
                    let testups = do
                            k <- ["a", "b", "c", "d"]
