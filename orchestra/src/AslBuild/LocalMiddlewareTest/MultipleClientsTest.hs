...
setups = do
    let rtime = 1
    let serverFlags = MemcachedFlags {memcachedPort = 11248, memcachedAsDaemon = False}
    nrClients <- [2,6 .. 20]
    keySize <- [16]
    valueSize <- [128]
    threads <- [2]
    ...
    let mwFlags =
            ...
            , mwVerbosity = LogFine
            , mwTraceFile =
                  tmpDir </> localMiddlewareMultipleClientsTestRule </>
                  localMiddlewareMultipleClientsTestRule ++
                  "-trace-" ++ signature <.> csvExt
    ...
    let flags =
            ...
            , msWindowSize = Kilo 1
            , msConfigFile =
                  tmpDir </> "local-middleware-multiple-clients-test" </>
                  "local-middleware-multiple-clients-test-memaslap-cfg-" ++
                  signature
            }
    let msSets = MemaslapSettings {msConfig = mconfig, msFlags = flags}
    return
