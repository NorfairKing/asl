...
data HighLevelConfig = HighLevelConfig
    { target :: String
    , nrClients :: Int
    , nrServers :: Int
    , location :: Location
...
data ExperimentSetup = ExperimentSetup
    ...
    , backendSetup :: Either ServerSetup (MiddleSetup, [ServerSetup])
...
data ExperimentResultSummary = ExperimentResultSummary
...
instance ToJSON ExperimentResultSummary where
    toJSON ExperimentResultSummary {..} =
        object
...
instance FromJSON ExperimentResultSummary where
    parseJSON (Object o) = do
        ...
        return
            ...
            {merMiddleResultsFile = mrf, erSetupFile = setup, erClientLogFiles = clrfs}
    parseJSON _ = mempty

data MiddleResultLine = MiddleResultLine
...
instance FromNamedRecord MiddleResultLine where
    parseNamedRecord r =
        MiddleResultLine <$> CSV.lookup r "Kind" <*> CSV.lookup r "ReceivedTime" <*>
