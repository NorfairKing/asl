...
data BaselineExperimentRuleCfg = BaselineExperimentRuleCfg
        ...
        let setups = do
                ...
                let defaultClients =
                        genClientSetup
                            ecf
                            cls
                            (RemoteServerUrl serPrivateIp .
                             memcachedPort . sMemcachedFlags . (\[s] -> s) . servers)
                            signGlobally
                            blRuntime
                let clients =
                        modif defaultClients $ \defcs ->
                            flip map defcs $ \cs ->
                                ...
                                in cs
                                   { cMemaslapSettings =
                                         sets
                                         { msConfig = config {setProportion = 0}
                                         , msFlags = flags {msConcurrency = curConcurrency}
                                         }
                                   }
                return $
...
genBaselineExperimentSetup
    ...
    -> (String -> FilePath)
    -> [ExperimentSetup]
genBaselineExperimentSetup ecf runtime clients server signGlobally =
    flip map [1 .. repititions (highLevelConfig ecf)] $ \r ->
        ExperimentSetup
        { esRuntime = runtime
        , esResultsSummaryFile =
              experimentResultsDir ecf </> "summaries" </> signGlobally "summary" </> "rep-" ++
              show r <.> jsonExt
        , esSetupFile =
              experimentResultsDir ecf </> "setups" </> signGlobally "setup" </> "rep-" ++
              show r <.> jsonExt
