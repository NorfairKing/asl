...
allExtremeExperiments = [smallLocalExtreme, localExtreme, smallRemoteExtreme, remoteExtreme]

smallLocalExtreme :: ExtremeCfg
smallLocalExtreme =
    smallRemoteExtreme
    {hlConfig = (hlConfig smallRemoteExtreme) {target = "small-local-extreme", location = Local}}

localExtreme :: ExtremeCfg
localExtreme =
    remoteExtreme
    { hlConfig =
          (hlConfig remoteExtreme)
          {target = "local-extreme", resultsPersistence = Volatile, location = Local}
    }
...
smallRemoteExtreme =
    ...
    { hlConfig =
          (hlConfig remoteExtreme)
          {target = "small-remote-extreme", resultsPersistence = Volatile, repititions = 1}
    , exRuntime = Seconds 5
    }
