...
data ThinkTimeCfg = ThinkTimeCfg
    ...
    genExperimentSetups ttc@ThinkTimeCfg {..} = do
        ...
        let middle =
                modif defaultMiddle $ \m ->
                    ...
                    { mMiddlewareFlags =
                          (mMiddlewareFlags m)
                          {mwReadSampleRate = Just 1, mwWriteSampleRate = Just 1, mwNrThreads = 1}
                    }
        let defaultClients =
                genClientSetup ttc [mid] (middleRemoteServer . middle) signGlobally ttRuntime
        let clients =
                modif defaultClients $ \defcs ->
                    flip map defcs $ \cs ->
                        ...
                        in cs
                           { cMemaslapSettings =
                                 sets
                                 { msConfig = config {setProportion = 0.01}
                                 , msFlags = flags {msConcurrency = 1}
                                 }
                           }
