...
startClientsOn clientSetups =
    parScriptAtResult $
    flip map clientSetups $ \ClientSetup {..} ->
        let s =
                script
                    [ "mkdir -p " ++ takeDirectory cRemoteLog
                    , unwords $
                      remoteMemaslap :
                      memaslapArgs (msFlags cMemaslapSettings) ++ [">", cRemoteLog, "2>&1"]
                    ]
        in (cRemoteLogin, s)
...
copyClientLogsBack clientSetups = do
    ...
    phPar clientSetups $ \ClientSetup {..} -> rsyncFrom cRemoteLogin cRemoteLog cLocalLog

shutdownClients :: [ClientSetup] -> Action ()
shutdownClients cs =
    phPar (nub $ map cRemoteLogin cs) $ \cRemoteLogin ->
        scriptAt cRemoteLogin $
        namedScript "kill-memaslap" [unwords ["killall", "memaslap", "||", "true"]]
...
waitAndWaitForClients runtime clientPhs = do
  ...
  where
    checkClients = do
        ...
        pure $
            case catMaybes $ checkAnyFailure mecs of
...
checkAnyFailure mecs =
    flip map (indexed mecs) $ \(cix, mec) ->
        case mec of
            Nothing -> Nothing
            Just ExitSuccess -> Nothing
            Just (ExitFailure e) ->
                Just $ unwords ["Client", show cix, "failed with exit code", show e]
            ...
            [] ->
                if all isJust mecs
                    then return ExperimentSuccess
                        ...
                        void $ liftIO $ timeout (5 * 1000 * 1000) $ forM_ phs waitForProcess
                        go
