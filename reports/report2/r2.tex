...
The so-called replication coefficient is a fraction that represents the ratio of redundant replication to the total number of backend servers. For example: For a setup with a total of $7$ backend servers and replication factor $4$, the replication coefficient is $\frac{1}{2}$
...
The details of this experiment's setups can be found in Figure \ref{fig:sut-replication-effect}.
...
Again we expect the total response time for write requests to increase.
...
In figure \ref{fig:replication-analysis-id}, there are grouped bar plots of response times \footnote{as measured by memaslap}, grouped by the replication factor.
...
In figure \ref{fig:replication-analysis-rev}, there are grouped bar plots of response time, grouped by the number of backend servers.
On each plot we again see that write requests incur a greater response time than read requests.

We also see that response times for both kinds of requests increase ever so slightly as the number of backend servers increases.
...
In the case of replication coefficients $0.5$ and $1.0$, we see that response times for read requests become only slightly longer while there is a big increase in the response times for write requests.
...
Each plot accounts for a fixed number of servers, with the replication factor on the x-axis.
...
In this last experiment, we investigate the results of increasing the percentage of the workload that consists of write requests on performance.
...
Moreover, we expect this difference to be even greater if more servers are used.
...
In Figure \ref{fig:write-analysis}, the throughput and response time of the middleware is plotted in function of the write percentage of the clients' workloads for fixed numbers of backend servers.

The plots clearly show that throughput and response time both stay constant as the write percentage increases in the case of no replication.
