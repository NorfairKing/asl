...
In fact, this can be show by looking at the ratio between the number of jobs in the queue and the number of jobs in the system.
...
In figure \ref{fig:tps-sign-table-add-vars}
...
This is consistent with the common knowledge \cite{} that the network is one of the most expensive parts of a distributed system.
Indeed, the higher the replication coefficient, the more servers ever write request has to hit, the longer it takes for a request to be handled, the fewer requests can be handled in a given amount of time time, the lower the throughput.

The next most important factor is the write percentage.
This makes sense because write requests are more expensive than read requests, especially if replication is used (see above).
...
This is because writes become a lot more expensive if replication is used, but writes need not be much more expensive than reads if no replication is used.
...
For completeness: A multiplicative model was tried as well, but because difference between the maximum $y$ and the minimum $y$ is small so it gave similar results and the additive model already has little variation explained by error.
