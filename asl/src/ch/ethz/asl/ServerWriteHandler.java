...
public class ServerWriteHandler {
    ...
    class ReadCompletionHandler implements CompletionHandler<Integer, ByteBuffer> {
        ...
        public void completed(Integer bytesRead, ByteBuffer bbuf) {
            if (ServerWriteHandler.this.serverHandler.isShuttingDown()) {
                log.fine("Shutting down writer read completion handler for server: " + serverAddress.getSocketAddress());
                return;
            }
            ...
            if (bytesRead <= 0) {
                resp = new ServerErrorResponse(
                        bytesRead + " bytes read from server: " + serverAddress.getSocketAddress());
            } else {
                log.finest("Input from Server:");
                log.finest(bytesRead + " bytes");
                resp = new SuccessfulResponse(ByteBuffer.wrap(bbuf.array(), 0, bytesRead));
            }
